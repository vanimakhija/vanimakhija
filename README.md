<div align="center">

# 👋 Hi, I'm Vani Makhija


### Software Developer • AI & Machine Learning • Data Analytics
### Building intelligent software that transforms data into impactful solutions.

<div align="center">
  <img src="./outputt.gif" width="40%">
</div>

<br>

<a href="https://github.com/vanimakhija">
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

<a href="https://www.linkedin.com/in/vanimakhija">
<img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="mailto:makhijavani2005@gmail.com">
<img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

</div>

<div align="center">

<img src="./Assets/output.gif" width="100%"/>

</div>

# 💫 About Me

🎓 Final Year B.Tech Computer Science Engineering (Data Science) at **VIT Chennai** (CGPA: **8.71/10**)

💡 I enjoy transforming ideas into real-world software by combining **Artificial Intelligence, Machine Learning, Data Analytics, and Full-Stack Development**.

🏆 Oracle Cloud Infrastructure 2025 AI Foundations Associate

🧩 Solved **300+ Data Structures & Algorithms** problems while strengthening problem-solving and software engineering skills.

🚀 Passionate about developing scalable applications, exploring AI-driven solutions, and continuously learning modern technologies.

---

## 🌟 What Defines Me

- 💻 Building end-to-end software solutions
- 🤖 Applying AI & Machine Learning to real-world problems
- 📊 Turning data into actionable insights
- 🌐 Developing modern web applications and APIs
- 🧠 Continuously learning and improving through hands-on projects


# 🎓 Education

### Vellore Institute of Technology, Chennai

**B.Tech — Computer Science Engineering (Data Science)**

📅 2023 – 2027

📊 CGPA: **8.71/10**


# 🎯 Focus Areas

🧠 Artificial Intelligence & Machine Learning
💻 Software Engineering
📊 Data Analytics & Business Intelligence
⚡ Backend Development
🌐 Full-Stack Development
🧩 Problem Solving & Algorithms

<p align="center">

<img src="https://skillicons.dev/icons?i=java,python,js,r,html,css"/>

</p>

<p align="center">

<img src="https://skillicons.dev/icons?i=java,python,js,r,html,css"/>

</p>

<p align="center">

<img src="https://skillicons.dev/icons?i=react,nextjs"/>

</p>
<p align="center">

<img src="https://skillicons.dev/icons?i=react,nextjs"/>

</p>
<p align="center">

<img src="https://skillicons.dev/icons?i=postgres,mysql,mongodb,sqlite"/>

</p>
<p align="center">

<img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas"/>

<img src="https://img.shields.io/badge/Numpy-013243?style=for-the-badge&logo=numpy"/>

<img src="https://img.shields.io/badge/Scikit--Learn-F7931E?style=for-the-badge&logo=scikitlearn"/>

<img src="https://img.shields.io/badge/PowerBI-F2C811?style=for-the-badge&logo=powerbi"/>

</p>
 🚀 Featured Projects

## 🌍 WanderGenie — AI Travel Planner

**Next.js • FastAPI • LLaMA 3.3 70B • OpenWeatherMap API**

- Developed a full-stack AI travel planning platform using Next.js and FastAPI with REST APIs.
- Integrated OpenWeatherMap APIs for personalized travel recommendations.
- Deployed using Vercel and Render.

---

## 🛡️ NetGuardAI — Real-Time Network Intrusion Detection

**Python • Flask • Random Forest • Docker • WebSockets**

- Built a machine learning pipeline using the UNSW-NB15 dataset (258K+ records).
- Achieved **95.2% multiclass attack detection accuracy**.
- Developed a real-time Flask dashboard with WebSockets and Docker.

---

## 🔐 Emotion Encryption

**Next.js • FastAPI • AES • JWT • Google OAuth • SQLite**

- Built a secure messaging platform with AES encryption and JWT authentication.
- Integrated Google OAuth 2.0.
- Developed REST APIs for authentication and secure messaging.

---

## 🛒 Retail Customer Shopping Trends Analysis

**Python • Pandas • SQL • PostgreSQL • Power BI**

- Analyzed **3900+ customer transactions**.
- Built interactive Power BI dashboards.
- Generated customer segmentation and business insights.

---

## 🩺 Digital Twin for Breast Cancer Diagnosis

**R • Shiny • Decision Tree • caret • dplyr • ggplot2**

- Developed an interactive Digital Twin dashboard using R Shiny.
- Implemented What-If analysis.
- Built confidence visualization and patient similarity analysis.

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=vanimakhija&theme=tokyo-night&hide_border=true"/>

</div>

# 🤝 Let's Connect

<div align="center">

<a href="https://github.com/vanimakhija">
<img src="https://img.shields.io/badge/GitHub-black?style=for-the-badge&logo=github"/>
</a>

<a href="https://www.linkedin.com/in/vanimakhija">
<img src="https://img.shields.io/badge/LinkedIn-blue?style=for-the-badge&logo=linkedin"/>
</a>

<a href="mailto:makhijavani2005@gmail.com">
<img src="https://img.shields.io/badge/Gmail-red?style=for-the-badge&logo=gmail"/>
</a>

</div>

